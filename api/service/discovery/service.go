...
func (s *Service) StartService() {
	utils.GetLogger().Info("Starting discovery service.")
	s.Init()
	s.Run()
}
...
func (s *Service) StopService() {
	utils.GetLogger().Info("Shutting down discovery service.")
	s.stopChan <- struct{}{}
	utils.GetLogger().Info("discovery service stopped.")
}
...
func (s *Service) NotifyService(params map[string]interface{}) {
	...
	if !ok {
		utils.GetLogger().Error("Wrong data type passed to NotifyService")
		return
	}

	utils.GetLogger().Info("[DISCOVERY]", "got notified", action)
	s.actionChan <- action
}
...
func (s *Service) contactP2pPeers() {
	...
	utils.GetLogger().Error("Constructing Ping Message", "myPing", pingMsg)
	...
	for {
		...
		case peer, ok := <-s.peerChan:
			if !ok {
				utils.GetLogger().Debug("[DISCOVERY] No More Peer!")
				break
			}
			...
			// utils.GetLogger().Debug("[DISCOVERY]", "add outgoing peer", peer)
		case <-s.stopChan:
			utils.GetLogger().Debug("[DISCOVERY] stop pinging ...")
			return
...
func (s *Service) sentPingMessage(g p2p.GroupID, msgBuf []byte) {
	...
	if err != nil {
		utils.GetLogger().Error("Failed to send ping message", "group", g)
	}
}

// Init is to initialize for discoveryService.
func (s *Service) Init() {
	utils.GetLogger().Info("Init discovery service")
}
