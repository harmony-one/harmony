...
func NewDownloaders(host p2p.Host, bcs []core.BlockChain, dbDir string, config Config, c *consensus.Consensus) *Downloaders {
	...
	for _, bc := range bcs {
		...
		ds[bc.ShardID()] = NewDownloader(host, bc, dbDir, isBeaconNode, config, c)
	}
	return &Downloaders{
		ds:     ds,
		active: abool.New(),
