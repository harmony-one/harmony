...
func NewService(host p2p.Host, bcs []core.BlockChain, config Config, dbDir string, c *consensus.Consensus) *StagedStreamSyncService {
	return &StagedStreamSyncService{
		Downloaders: NewDownloaders(host, bcs, dbDir, config, c),
	}
}
