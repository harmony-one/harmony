...
func NewDownloaders(host p2p.Host, bcs []core.BlockChain, nodeConfig *nodeconfig.ConfigType, consensus *consensus.Consensus, dbDir string, config Config, setNodeSyncStatus func(bool)) *Downloaders {
	...
	for _, bc := range bcs {
		...
		ds[bc.ShardID()] = NewDownloader(host, bc, nodeConfig, consensus, dbDir, isBeaconNode, config, setNodeSyncStatus)
	}
	return &Downloaders{
		ds:     ds,
		active: abool.New(),
