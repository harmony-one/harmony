...
func ClientSetup(ip, port string) *Client {
	...
	if err != nil {
		utils.GetLogInstance().Error("client.go:ClientSetup fail to dial: ", "error", err)
		return nil
	}
...
func (client *Client) Close() {
	...
	if err != nil {
		utils.GetLogInstance().Error("unable to close connection ")
	}
}
...
func (client *Client) GetBlockHashes(startHash []byte) *pb.DownloaderResponse {
	...
	if err != nil {
		utils.GetLogInstance().Error("[SYNC] GetBlockHashes query failed", "error", err)
	}
	return response
}
...
func (client *Client) GetBlocks(hashes [][]byte) *pb.DownloaderResponse {
	...
	response, err := client.dlClient.Query(ctx, request)
	if err != nil {
		utils.GetLogInstance().Error("[SYNC] downloader/client.go:GetBlocks query failed.", "error", err)
	}
	return response
}
...
func (client *Client) Register(hash []byte, ip, port string) *pb.DownloaderResponse {
	...
	if err != nil || response == nil {
		utils.GetLogInstance().Error("[SYNC] client.go:Register failed.", "error", err, "response", response)
	}
	return response
}
...
func (client *Client) PushNewBlock(selfPeerHash [20]byte, blockHash []byte, timeout bool) *pb.DownloaderResponse {
	...
	response, err := client.dlClient.Query(ctx, request)
	if err != nil {
		utils.GetLogInstance().Error("[SYNC] unable to send new block to unsync node", "error", err)
	}
	return response
}
...
func (client *Client) GetBlockChainHeight() *pb.DownloaderResponse {
	...
	response, err := client.dlClient.Query(ctx, request)
	if err != nil {
		utils.GetLogInstance().Error("[SYNC] unable to get blockchain height", "error", err)
	}
	return response
}
