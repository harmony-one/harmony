...
pushLoop:
	for {
		...
		if shardID != 0 {
			if otherHeight, flag := txGen.IsSameHeight(); flag {
				if otherHeight >= 1 {
					go func() {
						readySignal <- uint32(shardID)
						utils.Logger().Error().Msg("Same blockchain height so readySignal generated")
		...
		case shardID := <-readySignal:
			...
			if err != nil {
				utils.Logger().Error().Err(err).Msg("Error in Generating Txns")
			}
			lock.Lock()
...
func SendTxsToShard(clientNode *node.Node, txs types.Transactions, shardID uint32) {
	...
	if err != nil {
		utils.Logger().Error().Err(err).Msg("Error in Sending Txns")
	}
}
