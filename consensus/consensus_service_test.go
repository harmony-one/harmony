...
func TestPopulateMessageFields(t *testing.T) {
	...
	consensus, err := New(
		host, values.BeaconChainShardID, leader, nodeconfig.ToMultiPriKey(blsPriKey), decider,
	)
	if err != nil {
...
func TestSignAndMarshalConsensusMessage(t *testing.T) {
	...
	msg := &msg_pb.Message{}
	marshaledMessage, err := consensus.signAndMarshalConsensusMessage(msg, privateKey)
...
func TestSetViewID(t *testing.T) {
	...
	decider := quorum.NewDecider(quorum.SuperMajorityVote)
	consensus, err := New(
		host, values.BeaconChainShardID, leader, nodeconfig.ToMultiPriKey(bls.RandPrivateKey()), decider,
	)
	if err != nil {
		t.Fatalf("Cannot craeate consensus: %v", err)
	}
