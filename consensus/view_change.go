...
func (consensus *Consensus) startViewChange(viewID uint32) {
	...
	utils.GetLogInstance().Info("startViewChange", "viewID", viewID, "timeoutDuration", duration, "nextLeader", consensus.LeaderPubKey.SerializeToHexStr())
...
func (consensus *Consensus) onViewChange(msg *msg_pb.Message) {
	...
	if len(consensus.viewIDSigs) >= consensus.Quorum() {
		...
		utils.GetLogger().Debug("I am the new leader", "myKey", consensus.PubKey.SerializeToHexStr(), "viewID", consensus.viewID, "block", consensus.blockNum)
	}
...
func (consensus *Consensus) onNewView(msg *msg_pb.Message) {
	...
	if consensus.blockNum != recvMsg.BlockNum {
		utils.GetLogger().Debug("new leader changed", "newLeaderKey", consensus.LeaderPubKey.SerializeToHexStr(), "viewID", consensus.viewID, "myBlock", consensus.blockNum, "newViewBlockNum", recvMsg.BlockNum)
		return
	}
	...
	utils.GetLogger().Debug("new leader changed", "newLeaderKey", consensus.LeaderPubKey.SerializeToHexStr(), "viewID", consensus.viewID, "block", consensus.blockNum)
