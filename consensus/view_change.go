...
func (consensus *Consensus) onViewChange(msg *msg_pb.Message) {
	...
	if !(ok1 || ok2) {
		...
		if preparedMsg == nil {
			...
			consensus.nilSigs[recvMsg.ViewID][newLeaderKey.SerializeToHexStr()] = newLeaderPriKey.SignHash(NIL)
			...
			consensus.bhpSigs[recvMsg.ViewID][newLeaderKey.SerializeToHexStr()] = newLeaderPriKey.SignHash(msgToSign)
	...
	if !ok3 {
		...
		consensus.viewIDSigs[recvMsg.ViewID][newLeaderKey.SerializeToHexStr()] = newLeaderPriKey.SignHash(viewIDBytes)
