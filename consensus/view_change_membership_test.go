...
import (
	...
	blscore "github.com/harmony-one/harmony/crypto/bls/core"
...
func TestViewChangeSanityCheckRejectsNonCommitteeSender(t *testing.T) {
	...
	viewIDBytes := make([]byte, 8)
	binary.LittleEndian.PutUint64(viewIDBytes, viewID)
	message := func(key bls.PublicKeyWrapper, signature *blscore.Sign) *FBFTMessage {
		return &FBFTMessage{
			ViewID:        viewID,
