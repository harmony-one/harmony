...
func CalculateNewShardState(bc *BlockChain, epoch *big.Int) (shard.SuperCommittee, error) {
...
var (
	...
	GenesisCommitteeAssigner Assigner                = genesisPolicy{}
)
...
func CalculateInitShardState() shard.SuperCommittee {
	return CalculateShardState(big.NewInt(GenesisEpoch), GenesisCommitteeAssigner)
}
...
func CalculateShardState(epoch *big.Int, assign Assigner) shard.SuperCommittee {
	utils.Logger().Info().Int64("epoch", epoch.Int64()).Msg("Get Shard State of Epoch.")
	return assign.NextCommittee(ShardingSchedule.InstanceForEpoch(epoch))
}

// CalculatePublicKeys returns the publickeys given epoch and shardID
func CalculatePublicKeys(epoch *big.Int, shardID uint32, assign Assigner) []*bls.PublicKey {
