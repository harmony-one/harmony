...
import (
	...
	hcommon "github.com/harmony-one/harmony/common"
...
func TestStateProcessorErrors(t *testing.T) {
	...
	var makeTx = func(key *ecdsa.PrivateKey, nonce uint64, to common.Address, amount *big.Int, gasLimit uint64, gasPrice *big.Int, data []byte) *types.Transaction {
		tx, _ := types.SignTx(types.NewTransaction(nonce, to, 0, amount, gasLimit, gasPrice, data), signer, key)
		return tx
	}
	...
	{ // Tests against a 'recent' chain definition
		var (
			...
			blockchain, err = NewBlockChain(
				db, nil, nil,
				nil, nil, nil, vm.Config{})
			//tooBigInitCode = [params.MaxInitCodeSize + 1]byte{}
		)
		require.NoError(t, err)

		defer blockchain.Stop()
		bigNumber := new(big.Int).SetBytes(hcommon.MaxHash.Bytes())
		...
		for i, tt := range []struct {
			...
			db := rawdb.NewMemoryDatabase()
			block := GenerateBadBlock(gspec.ToBlock(db), common.Big1, beacon.New(ethash.NewFaker()), tt.txs, gspec.Config, false)
			_, err := blockchain.InsertChain(types.Blocks{block}, true)
			if err == nil {
	...
	*/
	...
	// ErrSenderNoEOA, for this we need the sender to have contract code
	{
		var (
			...
			blockchain, _ = NewBlockChain(db, nil, nil, nil, config, chain.NewEngine(), vm.Config{})
		)
		defer blockchain.Stop()
		for i, tt := range []struct {
			txs  []*types.Transaction
			...
			db := rawdb.NewMemoryDatabase()
			_ = beacon.New(ethash.NewFaker())
			block := GenerateBadBlock(gspec.ToBlock(db), common.Big1, nil, tt.txs, gspec.Config, false)
			_, err := blockchain.InsertChain(types.Blocks{block}, true)
			if err == nil {
				t.Fatal("block imported without errors")
			}
...
func GenerateBadBlock(parent *types.Block, epoch *big.Int, engine consensus.Engine, txs types.Transactions, config *params.ChainConfig, isPOW bool) *types.Block {
	...
	//	})
	//}
	header := blockfactory.NewFactory(config).
		NewHeader(epoch)
	...
	header.SetTime(new(big.Int).Add(parent.Time(), big.NewInt(10)))
	//header
	...
	header.SetRoot(common.BytesToHash(hasher.Sum(nil)))
