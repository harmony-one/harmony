...
func (config *TxPoolConfig) sanitize() TxPoolConfig {
	...
	if conf.Rejournal < time.Second {
		utils.Logger().Warn().Dur("provided", conf.Rejournal).Dur("updated", time.Second).Msg("Sanitizing invalid txpool journal time")
		conf.Rejournal = time.Second
	}
	if conf.PriceLimit < 1 {
		utils.Logger().Warn().Uint64("provided", conf.PriceLimit).Uint64("updated", DefaultTxPoolConfig.PriceLimit).Msg("Sanitizing invalid txpool price limit")
		conf.PriceLimit = DefaultTxPoolConfig.PriceLimit
	}
	if conf.PriceBump < 1 {
		utils.Logger().Warn().Uint64("provided", conf.PriceBump).Uint64("updated", DefaultTxPoolConfig.PriceBump).Msg("Sanitizing invalid txpool price bump")
...
type TxPool struct {
	...
	queueTxEventCh  chan types.PoolTransaction
...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain blockChain,
	txErrorSink *types.TransactionErrorSink,
) *TxPool {
	...
	pool := &TxPool{
		...
		queueTxEventCh:  make(chan types.PoolTransaction),
	...
	for {
		...
		case <-report.C:
			...
			if pending != prevPending || queued != prevQueued || stales != prevStales {
				utils.Logger().Debug().Int("executable", pending).Int("queued", queued).Int("stales", stales).Msg("Transaction pool status report")
...
func (pool *TxPool) SetGasPrice(price *big.Int) {
	...
	if price.Cmp(old) > 0 {
		// We iterate only remotes, so no need provide locals.
...
func (pool *TxPool) State() *txNoncer {
	pool.mu.RLock()
	defer pool.mu.RUnlock()

	return pool.pendingNonces
}
...
func (pool *TxPool) ContentFrom(addr common.Address) (types.PoolTransactions, types.PoolTransactions) {
	pool.mu.RLock()
	defer pool.mu.RUnlock()

	var pending types.PoolTransactions
	if list, ok := pool.pending[addr]; ok {
		pending = list.Flatten()
	}
	var queued types.PoolTransactions
...
func (pool *TxPool) Pending() map[common.Address]types.PoolTransactions {
	pool.mu.Lock()
	defer pool.mu.Unlock()
...
// account and sorted by nonce. The returned transaction set is a copy and can be
// freely modified by calling code.
func (pool *TxPool) Queued() map[common.Address]types.PoolTransactions {
	pool.mu.Lock()
	defer pool.mu.Unlock()
...
func (pool *TxPool) validateTx(tx types.PoolTransaction, local bool) error {
	...
	if !isStakingTx || (isStakingTx && stakingTx.StakingType() != staking.DirectiveDelegate) {
		...
		if balance.Cmp(cost) < 0 {
			costF := new(big.Float).SetInt64(cost.Int64())
			costF = costF.Mul(costF, new(big.Float).SetFloat64(1e-9)) // Gas-price is in Nano
			return errors.Wrapf(
				ErrInsufficientFunds,
				"current shard-id: %d, balance: %.18f, cost: %.18f",
	...
	if tx.GasLimit() < intrGas {
		return errors.WithMessagef(ErrIntrinsicGas, "transaction gas is %d, expected no less than %d", tx.GasLimit(), intrGas)
	}
...
func (pool *TxPool) add(tx types.PoolTransaction, local bool) (bool, error) {
	...
	if pool.all.Get(hash) != nil {
		...
		knownTxMeter.Mark(1)
		return false, errors.WithMessagef(ErrAlreadyKnown, "transaction hash %x", hash)
	}
	...
	if uint64(pool.all.Slots()+numSlots(tx)) > pool.config.GlobalSlots+pool.config.GlobalQueue {
		...
		if !isLocal && !success {
			logger.Debug().Str("hash", hash.Hex()).Msg("Discarding overflown transaction")
	...
	if list := pool.pending[from]; list != nil && list.Overlaps(tx) {
		...
		logger.Debug().Str("hash", hash.Hex()).Str("from", from.String()).Str("to", tx.To().String()).
...
func (pool *TxPool) enqueueTx(hash common.Hash, tx types.PoolTransaction, local bool, addAll bool) (bool, error) {
	...
	if pool.all.Get(hash) == nil && !addAll {
		utils.Logger().Error().Str("hash", hash.Hex()).
			Msg("Missing transaction in lookup set, please report the issue")
	}
	if addAll {
...
func (pool *TxPool) AddLocal(tx types.PoolTransaction) error {
	errs := pool.AddLocals([]types.PoolTransaction{tx})
	return errs[0]
}
...
func (pool *TxPool) AddRemotes(txs []types.PoolTransaction) []error {
	return pool.addTxs(txs, false, false)
}

// AddRemotesSync is like AddRemotes, but waits for pool reorganization. Tests use this method.
func (pool *TxPool) AddRemotesSync(txs []types.PoolTransaction) []error {
	return pool.addTxs(txs, false, true)
}

// This is like AddRemotes with a single transaction, but waits for pool reorganization. Tests use this method.
func (pool *TxPool) addRemoteSync(tx types.PoolTransaction) error {
	errs := pool.AddRemotesSync([]types.PoolTransaction{tx})
	return errs[0]
}
...
func (pool *TxPool) AddRemote(tx types.PoolTransaction) error {
	errs := pool.AddRemotes([]types.PoolTransaction{tx})
	return errs[0]
}

// addTxs attempts to queue a batch of transactions if they are valid.
func (pool *TxPool) addTxs(txs []types.PoolTransaction, local, sync bool) []error {
	// Filter out known ones without obtaining the pool lock or recovering signatures
	var (
		errs = make([]error, len(txs))
		news = make([]types.PoolTransaction, 0, len(txs))
	)
	for i, tx := range txs {
...
func (pool *TxPool) addTxsLocked(txs []types.PoolTransaction, local bool) ([]error, *accountSet) {
	dirty := newAccountSet(pool.chainconfig.ChainID)
	errs := make([]error, len(txs))
	for i, tx := range txs {
		...
		if err != nil && errors.Cause(err) != ErrAlreadyKnown {
...
func (pool *TxPool) queueTxEvent(tx types.PoolTransaction) {
	select {
...
func (pool *TxPool) reset(oldHead, newHead *block.Header) {
	...
	if oldHead != nil && oldHead.Hash() != newHead.ParentHash() {
		...
		if depth := uint64(math.Abs(float64(oldNum) - float64(newNum))); depth > 64 {
			logger.Debug().Uint64("depth", depth).Msg("Skipping deep transaction reorg")
		} else {
			...
			if rem == nil {
				...
				if newNum >= oldNum {
					...
					logger.Warn().
						Str("old", oldHead.Hash().Hex()).
						Uint64("oldnum", oldNum).
						Str("new", newHead.Hash().Hex()).
						Uint64("newnum", newNum).
						Msg("Transaction pool reset with missing oldhead")
					return
				}
				...
				logger.Warn().
					...
					Str("new", newHead.Hash().Hex()).
					Uint64("newnum", newNum).
					Msg("Skipping transaction reset caused by setHead")
				...
				for rem.NumberU64() > add.NumberU64() {
					...
					if rem = pool.chain.GetBlock(rem.ParentHash(), rem.NumberU64()-1); rem == nil {
						utils.Logger().Error().Str("block", oldHead.Number().String()).Str("hash", oldHead.Hash().Hex()).Msg("Unrooted old chain seen by tx pool")
						return
					}
				}
				for add.NumberU64() > rem.NumberU64() {
					...
					if add = pool.chain.GetBlock(add.ParentHash(), add.NumberU64()-1); add == nil {
						utils.Logger().Error().Str("block", newHead.Number().String()).Str("hash", newHead.Hash().Hex()).Msg("Unrooted new chain seen by tx pool")
						return
					}
				}
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) []types.PoolTransaction {
	...
	for _, addr := range accounts {
		...
		logger.Debug().Int("count", len(forwards)).Msg("Removed old queued transactions")
		...
		logger.Debug().Int("count", len(drops)).Msg("Removed unpayable queued transactions")
		...
		logger.Debug().Int("count", len(promoted)).Msg("Promoted queued transactions")
		queuedGauge.Dec(int64(len(readies)))

		// Drop all transactions over the allowed limit
		var caps types.PoolTransactions
		if !pool.locals.contains(addr) {
			...
			for pending > pool.config.GlobalSlots && pool.pending[offenders[len(offenders)-2]].Len() > threshold {
				for i := 0; i < len(offenders)-1; i++ {
					...
					for _, tx := range caps {
						...
						utils.Logger().Warn().Str("hash", hash.Hex()).Msg("Removed fairness-exceeding pending transaction")
					}
...
func newAccountSet(chainID *big.Int, addrs ...common.Address) *accountSet {
	as := &accountSet{
...
func (as *accountSet) addTx(tx types.PoolTransaction) {
	if addr, err := tx.SenderAddress(); err == nil {
		as.add(addr)
	}
}
...
type txLookup struct {
	slots   int
	lock    sync.RWMutex
	locals  map[common.Hash]types.PoolTransaction
	remotes map[common.Hash]types.PoolTransaction
}

// newTxLookup returns a new txLookup structure.
func newTxLookup() *txLookup {
	return &txLookup{
		locals:  make(map[common.Hash]types.PoolTransaction),
		remotes: make(map[common.Hash]types.PoolTransaction),
	}
}
...
func (t *txLookup) Range(f func(hash common.Hash, tx types.PoolTransaction, local bool) bool, local bool, remote bool) {
	t.lock.RLock()
...
func (t *txLookup) GetLocal(hash common.Hash) types.PoolTransaction {
	t.lock.RLock()
	defer t.lock.RUnlock()
...
func (t *txLookup) GetRemote(hash common.Hash) types.PoolTransaction {
	t.lock.RLock()
	defer t.lock.RUnlock()
...
func (t *txLookup) Add(tx types.PoolTransaction, local bool) {
	t.lock.Lock()
	defer t.lock.Unlock()
...
func (t *txLookup) Remove(hash common.Hash) {
	...
	if !ok {
		utils.Logger().Warn().Str("hash", hash.Hex()).Msg("No transaction found to be deleted")
		return
	}
	t.slots -= numSlots(tx)
	slotsGauge.Update(int64(t.slots))
...
func numSlots(tx types.PoolTransaction) int {
