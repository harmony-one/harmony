...
import (
	...
	"github.com/harmony-one/harmony/hmy_boot/tracers"
...
func (hmyboot *BootService) TraceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithCache(hmyboot.ChainDb(), 16)

	if origin > 0 {
		start = hmyboot.BlockChain.GetBlock(start.ParentHash(), origin-1)
		if start == nil {
	...
	if err != nil {
		...
		for i := uint64(0); i < reexec; i++ {
			start = hmyboot.BlockChain.GetBlock(start.ParentHash(), start.NumberU64()-1)
			if start == nil {
				break
			}
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				hmySigner := types.MakeSigner(hmyboot.BlockChain.Config(), task.block.Number())
				ethSigner := types.NewEIP155Signer(hmyboot.BlockChain.Config().EthCompatibleChainID)
				...
				for i, tx := range task.block.Transactions() {
					...
					vmCtx := core.NewEVMContext(msg, task.block.Header(), hmyboot.BlockChain, nil)

					res, err := hmyboot.TraceTx(ctx, msg, vmCtx, task.statedb, config)
					if err != nil {
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := hmyboot.BlockChain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, _, _, _, _, err := hmyboot.BlockChain.Processor().Process(block, statedb, vm.Config{}, false)
			if err != nil {
				failed = err
				break
			}
...
func (hmyboot *BootService) traceBlockNoThread(ctx context.Context, block *types.Block, config *TraceConfig) ([]*TxTraceResult, error) {
	// Create the parent state database
	if err := hmyboot.BlockChain.Engine().VerifyHeader(hmyboot.BlockChain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := hmyboot.BlockChain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	statedb, err := hmyboot.ComputeStateDB(parent, reexec)
	if err != nil {
		return nil, err
	}
	// Execute all the transaction contained within the block concurrently
	var (
		hmySigner = types.MakeSigner(hmyboot.BlockChain.Config(), block.Number())
		ethSigner = types.NewEIP155Signer(hmyboot.BlockChain.Config().EthCompatibleChainID)
		txs       = block.Transactions()
...
traceLoop:
	for i, tx := range txs {
		...
		vmctx := core.NewEVMContext(msg, block.Header(), hmyboot.BlockChain, nil)
		res, err := hmyboot.TraceTx(ctx, msg, vmctx, statedb, config)
		if err != nil {
...
// executes all the transactions contained within. The return value will be one item
// per transaction, dependent on the requested tracer.
func (hmyboot *BootService) TraceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*TxTraceResult, error) {
	select {
	case <-ctx.Done():
	...
	if config != nil && config.Tracer != nil && *config.Tracer == "ParityBlockTracer" {
		return hmyboot.traceBlockNoThread(ctx, block, config)
	}
	// Create the parent state database
	if err := hmyboot.BlockChain.Engine().VerifyHeader(hmyboot.BlockChain, block.Header(), true); err != nil {
		return nil, err
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(vmctx, statedb, hmyboot.BlockChain.Config(), vm.Config{})
...
func (hmyboot *BootService) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		vmenv := vm.NewEVM(vmctx, statedb, hmyboot.BlockChain.Config(), vmConf)
...
func (hmyboot *BootService) ComputeStateDB(block *types.Block, reexec uint64) (*state.DB, error) {
	// If we have the state fully available, use that
	statedb, err := hmyboot.BlockChain.StateAt(block.Root())
	if err == nil {
		return statedb, nil
	}
	// Otherwise try to reexec blocks until we find a state or reach our limit
	origin := block.NumberU64()
	database := state.NewDatabaseWithCache(hmyboot.BlockChain.ChainDb(), 16)

	for i := uint64(0); i < reexec; i++ {
		block = hmyboot.BlockChain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = hmyboot.BlockChain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (hmyboot *BootService) TraceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.DB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, statedb, hmyboot.BlockChain.Config(), vm.Config{Debug: true, Tracer: tracer})
...
func (hmyboot *BootService) ComputeTxEnv(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.Context, *state.DB, error) {
	// Create the parent state database
	parent := hmyboot.BlockChain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), hmyboot.BlockChain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, statedb, hmyboot.BlockChain.Config(), vm.Config{})
...
func (hmyboot *BootService) ComputeTxEnvEachBlockWithoutApply(block *types.Block, reexec uint64, cb func(int, *types.Transaction, core.Message, vm.Context, *state.DB) bool) error {
	// Create the parent state database
	parent := hmyboot.BlockChain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
