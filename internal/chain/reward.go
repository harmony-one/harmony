...
func getDefaultStakingReward(bc engine.ChainReader, epoch *big.Int, blockNum uint64) numeric.Dec {
	...
	if bc.Config().ChainID == params.TestnetChainID && bc.Config().FiveSecondsEpoch.Cmp(big.NewInt(16500)) == 0 {
		...
		if bc.Config().IsHIP30(epoch) {
			defaultReward = stakingReward.HIP30TwoSecStakedBlocks
...
func AccumulateRewardsAndCountSigs(
	...
	if !bc.Config().IsAggregatedRewardEpoch(header.Epoch()) {
		return distributeRewardBeforeAggregateEpoch(
			bc, state, header, beaconChain, defaultReward, sigsReady,
		)
	}
	...
	return distributeRewardAfterAggregateEpoch(
		bc, state, header, beaconChain, defaultReward,
	)
}
...
func distributeRewardAfterAggregateEpoch(
	...
	rewardToDistribute numeric.Dec,
) (numeric.Dec, reward.Reader, error) {
	epoch := header.Epoch()
	...
	// If initial value of i underflows, the loop does not execute
