...
func (node *Node) CallFaucetContract(address common.Address) common.Hash {
	...
	utils.Logger().Info().Str("Sending placeholder token to ", common2.MustAddressToBech32(address))
...
func (node *Node) callGetFreeTokenWithNonce(address common.Address, nonce uint64) common.Hash {
	...
	if err != nil {
		utils.GetLogInstance().Error().Err(err).Msg("Failed to generate ABI function bytes data")
		return common.Hash{}
	}
	...
	utils.Logger().Info().Str("Sending Free Token to ", "Address", common2.MustAddressToBech32(address))
...
func (node *Node) AddContractKeyAndAddress(t builtInSC) {
	...
	default:
		utils.Logger().Error().Err(err).Msg("AddContractKeyAndAddress")
	}
}
