...
func (node *Node) validateNewShardState(block *types.Block) error {
	...
	if block.ShardID() == shard.BeaconChainShardID {
		...
		expected, err := committee.WithStakingEnabled.ReadFromComputation(
		...
		if shard.CompareSuperCommittee(expected, proposed) != 0 {
		...
		masterProposal := node.nextShardState.master.SuperCommittee
...
func (node *Node) broadcastEpochShardState(newBlock *types.Block) error {
	superCommittee, err := newBlock.Header().GetShardState()
	if err != nil {
		return err
	}
	epochShardStateMessage := proto_node.ConstructEpochShardStateMessage(
		shard.EpochSuperCommittee{
			newBlock.Header().Epoch().Uint64() + 1,
			superCommittee,
		},
	)
...
func (node *Node) epochShardStateMessageHandler(msgPayload []byte) error {
	epochShardState, err :=
		proto_node.DeserializeEpochShardStateFromMessage(msgPayload)
	if err != nil {
	...
	err = node.Beaconchain().WriteShardState(
		receivedEpoch, epochShardState.SuperCommittee,
	)
	if err != nil {
