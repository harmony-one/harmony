		...
		case beaconBlock := <-node.BeaconBlockChannel:
			...
			if node.beaconSync.GetActivePeerNumber() == 0 {
				...
				if err := node.beaconSync.CreateSyncConfig(peers, true); err != nil {
					ctxerror.Log15(utils.Logger().Debug, err)
					continue
				}
			}
		...
		case <-ticker.C:
			...
			if node.stateSync.GetActivePeerNumber() == 0 {
				...
				if err := node.stateSync.CreateSyncConfig(peers, false); err != nil {
					ctxerror.Log15(utils.Loger().Error().Err(err).Msg())
...
func (node *Node) StartSyncingServer() {
	utils.Logger().Info().Msg("support_syncing: StartSyncingServer")
...
func (node *Node) SendNewBlockToUnsync() {
	for {
		...
		utils.Logger().
		Info("selfPubKey", node.Consensus.PubKey.SerializeToHexStr()).
		Int("number", len(node.peerRegistrationRecord)).
		Msg("[SYNC] peerRegistration Record")
		...
		for peerID, config := range node.peerRegistrationRecord {
			...
			if elapseTime > broadcastTimeout {
				utils.Logger().Info("peerID", peerID).Msg("[SYNC] SendNewBlockToUnsync to peer timeout")
...
func (node *Node) CalculateResponse(request *downloader_pb.DownloaderRequest) (*downloader_pb.DownloaderResponse, error) {
	...
	case downloader_pb.DownloaderRequest_NEWBLOCK:
		if node.State != NodeNotInSync {
			utils.Logger().Info("state", node.State.String()).Msg("[SYNC] new block received, but state is")
		...
		if err != nil {
			utils.Logger().Info().Msg("[SYNC] unable to decode received new block")
			return response, err
		}
	...
	case downloader_pb.DownloaderRequest_REGISTER:
		...
		if _, ok := node.peerRegistrationRecord[peerID]; ok {
			...
			utils.Logger().Warn().Err(err).Msg("[SYNC] peerRegistration record already exists")
			return response, nil
		} else if len(node.peerRegistrationRecord) >= maxBroadcastNodes {
			response.Type = downloader_pb.DownloaderResponse_FAIL
			utils.Logger().Warn.Err(err).Msg("[SYNC] maximum registration limit exceeds")
			return response, nil
		} else {
			response.Type = downloader_pb.DownloaderResponse_FAIL
			...
			if client == nil {
				utils.Logger().Warn.Err(err).Msg("[SYNC] unable to setup client for peerID")
				return response, nil
			}
			...
			node.stateMutex.Unlock()
			utils.Logger().Debug().Msg("[SYNC] register peerID success")
	...
	case downloader_pb.DownloaderRequest_REGISTERTIMEOUT:
		if node.State == NodeNotInSync {
			...
			utils.Logger().Debug().Msg("[SYNC] extra node registered")
		}
	}
	return response, nil
}
