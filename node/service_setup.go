...
func (node *Node) setupForExplorerNode() {
	...
	node.serviceManager.RegisterService(service.SupportExplorer, explorer.New(&node.SelfPeer, node.Blockchain().ShardID(), node.GetBalanceOfAddress, node.Blockchain().ReadShardState))
	// Register explorer service.
}
