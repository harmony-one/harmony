...
func (node *Node) UpdateStakingList(stakeInfoReturnValue *structs.StakeInfoReturnValue) {
	utils.Logger().Info("contractState", stakeInfoReturnValue).Msg("Updating staking list")
...
func (node *Node) printStakingList() {
	...
	for addr, stakeInfo := range node.CurrentStakes {
		utils.Logger().Info("", "Address", addr, "BlsPubKey", hex.EncodeToString(stakeInfo.BlsPublicKey[:]), "BlockNum", stakeInfo.BlockNum, "lockPeriodCount", stakeInfo.LockPeriodCount, "amount", stakeInfo.Amount)
	}
