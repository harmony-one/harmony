...
EOF
  ...
  RB_SERVICE="$ABSENT_UNIT" run_rb_user hmytest "$INV" prepare
  expect "m_blspass/prepare" 0 "^READY $BLS_SORTED recovery-92730034$" || return 0
  : > "$T/require-bls-pass"
  ...
  expect "m_blspass/start" 0 "^RUNNING $BLS_SORTED recovery-92730034$" || return 0
...
EOF
  ...
  local pass_file="$INV/.hmy/blskeys/test.pass"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_blspassroot/prepare" 0 "^READY $BLS_SORTED recovery-92730034$" || return 0
  : > "$T/require-bls-pass"
  RB_SERVICE="$ABSENT_UNIT" run_rb_input test-pass "$INV" start
  expect "m_blspassroot/start" 0 "^RUNNING $BLS_SORTED recovery-92730034$" || return 0
...
EOF
  ...
  start_orig
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_blskms/prepare" 0 "^READY $BLS_SORTED recovery-92730034$" || return 0
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
  expect "m_blskms/start" 0 "^RUNNING $BLS_SORTED recovery-92730034$"
