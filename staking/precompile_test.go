...
var ParseMigrationMsgTests = []parseTest{
	{
...
func testParseMigrationMsg(test parseTest, t *testing.T) {
	t.Run(fmt.Sprintf("%s", test.name), func(t *testing.T) {
		if res, err := ParseMigrationMsg(common.HexToAddress("1337"), test.input); err != nil {
			if test.expectedError != nil {
				if test.expectedError.Error() != err.Error() {
					t.Errorf("Expected error %v, got %v", test.expectedError, err)
				}
			...
			if test.expected != nil {
				...
				} else if !res.Equals(*convertedExp) {
...
func TestParseMigrationMsgs(t *testing.T) {
	for _, test := range ParseMigrationMsgTests {
		testParseMigrationMsg(test, t)
	}
}
