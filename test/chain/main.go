...
func fundFaucetContract(chain *core.BlockChain) {
	...
	txs = append(txs, tx)
	err := contractworker.CommitTransactions(txs, transaction.Stakes{}, testUserAddress)
	if err != nil {
		fmt.Println(err)
	}
...
func callFaucetContractToFundAnAddress(chain *core.BlockChain) {
	...
	err = contractworker.CommitTransactions(types.Transactions{callfaucettx}, transaction.Stakes{}, testUserAddress)
	if err != nil {
		fmt.Println(err)
	}
	if err != nil {
...
func playStaking(chain *core.BlockChain) {
	...
	err = contractworker.CommitTransactions(stakingtxns, transaction.Stakes{}, common.Address{})
...
func playWithdrawStaking(chain *core.BlockChain) {
	...
	err = contractworker.CommitTransactions(withdrawstakingtxns, transaction.Stakes{}, common.Address{})
	if err != nil {
		fmt.Println("error:")
		fmt.Println(err)
	}
