...
function launch_localnet() {
  ...
  base_args=(--log_folder "${log_folder}" --min_peers "${MIN}" --bootnodes "${BN_MA}" "--network_type=$NETWORK" --blspass file:"${ROOT}/.hmy/blspass.txt" "--dns=false" "--sync.stagedsync=false" "--verbosity=${verbosity}" "--p2p.security.max-conn-per-ip=100")
  sleep 2
